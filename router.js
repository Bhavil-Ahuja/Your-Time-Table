...
router.get("/dashboard/edit", (req, res) => {
    Student.findOne({ email: req.session.user }, function (err, results) {
        res.render('edit_profile', { name: results.name, email: results.email, year: results.year, branch: results.branch, batch: results.batch })
    })
})

router.post("/dashboard/edit", (req, res) => {
    Student.findOneAndUpdate({ email: req.body.email }, { name: req.body.name, email: req.body.email, year: req.body.year, branch: req.body.branch, batch: req.body.batch }, function (err, results) {
        if (err) {
...
router.get("/time-table/:batch", function (req, res) {
    tt.findOne({ batch: req.params.batch }, function (err, results) {
        if (!err) {
            res.render("page", { batch: _.capitalize(req.params.batch), batch2: req.params.batch, data: results })
        }
    })
})
